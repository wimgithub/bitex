...
public class GoogleAuthenticationController extends BaseController{
    ...
    public MessageResult  sendgoogle(@SessionAttribute(SESSION_MEMBER) AuthMember user) {
        ...
        if(StringUtils.isEmpty(member.getEmail())){
            prefix=member.getId().toString();
        }else{
            prefix=member.getEmail().substring(0,member.getEmail().indexOf("@"));
        }
        String qrBarcodeURL = GoogleAuthenticatorUtil.getQRBarcodeURL(prefix,googleAuthUrl, secret);
