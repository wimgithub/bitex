...
import static com.spark.bitrade.constant.SysConstant.RESET_PASSWORD_CODE_PREFIX;
...
public class ApproveController {
    ...
    public MessageResult updateEmail(@SessionAttribute(SESSION_MEMBER) AuthMember user,String oldEmailCode,
        ...
        ValueOperations valueOperations = redisTemplate.opsForValue();
        Object oldEmailCache = valueOperations.get(SysConstant.EMAIL_UNTIE_CODE_PREFIX + member.getEmail());
        Object newEmailCache = valueOperations.get(SysConstant.EMAIL_UPDATE_CODE_PREFIX + newEmail);
        ...
        valueOperations.getOperations().delete(SysConstant.EMAIL_UNTIE_CODE_PREFIX + member.getEmail());
        valueOperations.getOperations().delete(SysConstant.EMAIL_UPDATE_CODE_PREFIX + newEmail);
    ...
    public MessageResult realApprove(String realName, String idCard, String idCardFront,
        ...
        List<MemberApplication> oldData=memberApplicationService.findByMemberAndAuditStatus(member,AuditStatus.AUDIT_ING);
        if(oldData!=null&&oldData.size()>0){
            return MessageResult.error(msService.getMessage("REPEAT_APPLICATION"));
        }
    ...
    private MessageResult checkCode(String key,Member member,String code,Integer codeMold){
        if(codeType==0){
            ...
            Object cache = valueOperations.get(fullKey);
            notNull(cache, msService.getMessage("NO_GET_VERIFICATION_CODE"));
            if (!code.equals(cache.toString())) {
                return MessageResult.error(msService.getMessage("VERIFICATION_CODE_INCORRECT"));
            } else {
                valueOperations.getOperations().delete(fullKey);
            }
        }else if(codeType==1){
