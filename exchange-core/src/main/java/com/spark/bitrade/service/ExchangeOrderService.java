...
import com.spark.bitrade.enums.PaymentType;
...
public class ExchangeOrderService extends BaseService {
     ...
     */
    @Transactional
    public MessageResult addOrder(Long memberId, ExchangeOrder order) throws SQLException, DataException {
        ...
        if (order.getDirection() == ExchangeOrderDirection.BUY) {
            ...
            DB.query("select id from member_wallet where id = ? for update;",wallet.getId());
     ...
     * @return
     */
    public Page<ExchangeOrder> findHistory(Long uid, String symbol, int pageNo, int pageSize,BooleanEnum marginTrade,
        ...
        if(startTime!=null){
            specification.add(Restrictions.gte("time",startTime.getTime(),true));
        }
        if(endTime!=null){
            specification.add(Restrictions.lt("time",endTime.getTime(),true));
        }
    ...
    public MessageResult processExchangeTrade(ExchangeTrade trade, boolean secondReferrerAward) throws Exception {
        ...
        if(buyOrder.getMemberId().compareTo(sellOrder.getMemberId()) != 0) {
     ...
     * @param order               委托订单
     * @param trade               交易详情
     * @param exchangeCoin        币对
     * @param secondReferrerAward 二级推荐人是否返佣
     * @return
     */
    public void processOrder(ExchangeOrder order, ExchangeTrade trade, ExchangeCoin exchangeCoin, boolean secondReferrerAward) {
        ...
        BigDecimal feeRatio;
        if (order.getDirection() == ExchangeOrderDirection.BUY) {
            feeRatio=exchangeCoin.getFee();
            turnover = trade.getBuyTurnover();
        } else {
            feeRatio=exchangeCoin.getBaseFee();
    ...
    /**
     * 根据memberIdList查询所有已完成和已取消的挂单
     *
     * @param memberId
     * @return
     */
    public List<ExchangeOrder> findAllCompletedOrderByMemberIdAndCoinAndDirection(Long memberId,String coinUnit,ExchangeOrderDirection direction) {
        if(direction==ExchangeOrderDirection.BUY){
            return exchangeOrderRepository.findAllBuyCompletedOrderByMemberIdAndCoin(memberId,coinUnit,direction);
        }else if(direction==ExchangeOrderDirection.SELL){
            return exchangeOrderRepository.findAllSellCompletedOrderByMemberIdAndCoin(memberId,coinUnit,direction);
        }
        return null;
    }
}
