...
public class WithdrawRecordController extends BaseAdminController {
    ...
    public MessageResult pageQuery(
        ...
        if(!StringUtils.isEmpty(screen.getMemberUsername())){
            predicates.add(QMember.member.username.like("%" + screen.getMemberUsername() + "%"));
        }
        if(!StringUtils.isEmpty(screen.getMemberRealName())){
            predicates.add(QMember.member.realName.like("%" + screen.getMemberRealName() + "%"));
        }
        if(!StringUtils.isEmpty(screen.getPhone())){
            predicates.add(QMember.member.mobilePhone.like("%"+screen.getPhone()+"%"));
        }
    ...
    @Transactional(rollbackFor = Exception.class)
    public MessageResult authWithdraw(@RequestParam("ids") Long[] ids) throws SQLException, DataException {
        ...
        for(WithdrawRecord withdrawRecord:withdrawRecordList){
            //加排它锁，防止重复提交
            DB.query("select a from WithdrawRecord a where a.id in = ? for update;",withdrawRecord.getId());
            isTrue(withdrawRecord.getStatus() == WAITING, "提现状态不是等待放币,不能打款!");
            isTrue(withdrawRecord.getIsAuto() == IS_FALSE, "不是人工审核提现!");
            try {
