...
import com.spark.bitrade.service.BusinessAuthApplyService;
import com.spark.bitrade.service.BusinessAuthDepositService;
...
public class BusinessAuthController extends BaseAdminController {
    ...
    @RequiresPermissions("business-auth:apply:page-query")
    @GetMapping("apply/out-excel")
    @AccessLog(module = AdminModule.MEMBER, operation = "导出商家认证 Excel")
    public void outExcel(PageModel pageModel,@RequestParam(value = "status", required = false) CertifiedBusinessStatus status,
                         @RequestParam(value = "account", defaultValue = "") String account,
                         HttpServletResponse response) throws Exception {
        ArrayList<Predicate> predicates = new ArrayList<>();
        if (!"".equals(account)) {
            predicates.add(QBusinessAuthApply.businessAuthApply.member.username.like("%" + account + "%")
                    .or(QBusinessAuthApply.businessAuthApply.member.mobilePhone.like(account + "%"))
                    .or(QBusinessAuthApply.businessAuthApply.member.email.like(account + "%"))
                    .or(QBusinessAuthApply.businessAuthApply.member.realName.like("%" + account + "%")));
        }
        if (status != null) {
            predicates.add(QBusinessAuthApply.businessAuthApply.certifiedBusinessStatus.eq(status));
        }
        pageModel.setPageSize(Integer.MAX_VALUE);
        Page<BusinessAuthApplyVO> page = businessAuthApplyService.outExcel(predicates, pageModel);
        List<BusinessAuthApplyVO> list=new ArrayList<>();
        ...
        ExcelUtil.listToExcel(list,BusinessAuthApplyVO.class.getDeclaredFields(),response.getOutputStream());
    }

    private void madeData(List<BusinessAuthApplyVO> list){
        for(BusinessAuthApplyVO vo:list){
            vo.setMemberLevelCN(vo.getMemberLevel().getCnName());
            vo.setCertifiedBusinessStatusCN(vo.getCertifiedBusinessStatus().getCnName());
            vo.setPublishAdvertiseCN(vo.getPublishAdvertise().getNameCn());
        }
    }
}
