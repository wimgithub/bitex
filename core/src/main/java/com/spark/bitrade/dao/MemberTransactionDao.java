...
public interface MemberTransactionDao extends BaseDao<MemberTransaction> {
    ...
    @Query("select sum(t.amount)  as amount from MemberTransaction t where  t.amount > 0 and t.memberId = :memberId and t.type in :types")
    Map<String,Object> findTransactionSum(@Param("memberId") Long memberId,@Param("types") List<TransactionType> types);
